...
Если новость помечена как `red: true` дополнительно её заголовок будет подсвечен красным.
...
Параметр `important:` в этих списках не учитывается, все новости идут строго по дате и времени, а параметр `red: true` - учитывается.
